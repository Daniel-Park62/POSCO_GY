...
UPDATE motestatus SET 
 ...
 abdate1 = case when errflag1 = 0 AND ( NEW.rtd1 <= 0 OR truncate(NEW.rtd1 + 0.001 ,1) = 1.9 OR NEW.rtd1 > 200 OR  errcnt1 > 2 ) then NEW.tm ELSE abdate1 END,
 errflag1 = case when errflag1 = 0 AND ( NEW.rtd1 <= 0 OR truncate(NEW.rtd1 + 0.001,1) = 1.9 OR NEW.rtd1 > 200 OR  errcnt1 > 2 ) then 1 ELSE errflag1 END,
 ...
 abdate2 = case when cntgb = 1 and errflag2 = 0 AND ( NEW.rtd2 <= 0 OR truncate(NEW.rtd2 + 0.001,1) = 1.9 OR NEW.rtd2 > 200 OR errcnt2 > 2 ) then NEW.tm ELSE abdate2 END,
 errflag2 = case when cntgb = 1 and errflag2 = 0 AND ( NEW.rtd2 <= 0 OR truncate(NEW.rtd2 + 0.001,1) = 1.9 OR NEW.rtd2 > 200 OR errcnt2 > 2 ) then 1 ELSE errflag2 END,
 ...
 abdate3 = case when cntgb = 1 AND (seq MOD 2) = 1 AND errflag3 = 0 AND ( NEW.rtd3 <= 0 OR truncate(NEW.rtd3 + 0.001,1) = 1.9 OR NEW.rtd3 > 200 OR errcnt3 > 2 ) then NEW.tm ELSE abdate3 END,
 errflag3 = case when cntgb = 1 AND (seq MOD 2) = 1 AND errflag3 = 0 AND ( NEW.rtd3 <= 0 OR truncate(NEW.rtd3 + 0.001,1) = 1.9 OR NEW.rtd3 > 200 OR errcnt3 > 2 ) then 1 ELSE errflag3 END,
