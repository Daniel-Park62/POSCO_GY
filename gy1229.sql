...
UPDATE motestatus SET 
 ...
 errflag1 = case when errflag1 = 0 AND ( NEW.rtd1 <= 0 OR round(NEW.rtd1 ,1) = 1.9 OR NEW.rtd1 > 200 OR ( ABS(prtd1 - NEW.rtd1) > 15 and errcnt1 > 2)) then 1 ELSE errflag1 END,
 ...
 errflag2 = case when cntgb = 1 and errflag2 = 0 AND ( NEW.rtd2 <= 0 OR round(NEW.rtd2 ,1) = 1.9 OR NEW.rtd2 > 200 OR ( ABS(prtd2 - NEW.rtd2) > 15 and errcnt2 > 2)) then 1 ELSE errflag2 END,
 ...
 errflag3 = case when cntgb = 1 AND (seq MOD 2) = 1 AND errflag3 = 0 AND ( NEW.rtd3 <= 0 OR round(NEW.rtd3 ,1) = 1.9 OR NEW.rtd3 > 200 OR ( ABS(prtd3 - NEW.rtd3) > 15 and errcnt3 > 2)) then 1 ELSE errflag3 END,
...
WHERE mmgb = NEW.mmgb AND seq = NEW.seq AND errflag1 = 1 AND prtd1 = 0 and NEW.rtd1 BETWEEN 2 AND 199  ;
UPDATE motestatus SET errflag2 = 0, errcnt2 = 0
WHERE mmgb = NEW.mmgb AND seq = NEW.seq AND errflag2 = 1 AND prtd2 = 0 and NEW.rtd2 BETWEEN 2 AND 199  ;
UPDATE motestatus SET errflag3 = 0, errcnt3 = 0
WHERE mmgb = NEW.mmgb AND seq = NEW.seq AND errflag3 = 1 AND prtd3 = 0 and NEW.rtd3 BETWEEN 2 AND 199  ;
