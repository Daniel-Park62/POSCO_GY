...
console.info("(GWIP_WS)", GWIP_WS, "(GWIP_DS)", GWIP_DS, "(MPORT)", port);
...
async function getMeasure() {
  await con.query("SELECT seq,act,bno, stand, batt , swseq, spare, gubun, cntgb FROM motestatus  where mmgb = '1' or cntgb = 1 ")
    .then(rows => { 
      rows.forEach((e, i) => { ws_ss[i] = []; ws_ss[i] = [e.act, e.bno, e.stand, e.batt, e.spare, e.gubun, e.seq, e.swseq ] });
    })
    .catch(err => console.error(err));

  await con.query("SELECT seq,act,bno,stand, batt , swseq, spare, gubun FROM motestatus  where mmgb = '2' and cntgb = 0 ")
    .then(rows => {
      rows.forEach((e, i) => { ds_ss[i] = []; ds_ss[i] = [e.act, e.bno, e.stand, e.batt, e.spare, e.gubun, e.seq, e.swseq ] });
    })
    .catch(err => console.error(err));

  await con.query("SELECT measure, batt FROM MOTECONFIG LIMIT 1")
    .then(row => {
      MEAS = row[0].measure;
      BATTL = row[0].batt;
    })
  ...
  ;
  await con.query("SELECT max(seq)  as ssnum FROM motestatus where spare = 'N' and GUBUN = 'S' ")
    .then(rows => SSNUM = rows[0].ssnum)
    .catch(err => SSNUM = 10)
    .finally(() => console.info('Sensor num :' + SSNUM));

  await con.query("SELECT sum(IF(MMGB='1',1,0)) as ws_num, sum(IF(MMGB='2',1,0)) as ds_num FROM motestatus where spare = 'N' ")
    .then(row => { WS_NUM = row[0].ws_num ;  DS_NUM = row[0].ds_num ; 
            console.info('W/S num :' , WS_NUM, ' D/S num :', DS_NUM)})
    .catch(err => WS_NUM = 0, DS_NUM = 0 ) ;

  await con.query("SELECT lastm FROM lastime LIMIT 1 ")
    .then(row => svtime = moment(row[0].lastm))
    .catch(err => { svtime = moment() ; console.log(err)})
    .finally(console.info('last time :' + svtime.format('YYYY-MM-DD HH:mm:ss')));
}
...
function getDevs() {
  ...
  cli_dev.connectTCP(GWIP_WS, { port: DEVPORT })
    ...
    // DS READ
    if (DS_NUM > 0 )
    cli_dev.connectTCP(GWIP_DS, { port: DEVPORT })
    .then(async () => {
...
async function insTemp() {
  ...
  await client.connectTCP(GWIP_WS, { port: TAGPORT })
    .then(async () => {
      ...
      await con.query('UPDATE lastime SET lastm = ? ', [tm])
    ...
    // DS READ
    if (DS_NUM > 0)
    client.connectTCP(GWIP_DS, { port: TAGPORT })
    .then(async () => {
      await client.setID(1);

      await client.readInputRegisters(1, 84)
        .then(async (d) => {
          rtags = new Uint16Array(d.data);
          let vrtd1 = 0, vrtd2 = 0, vrtd3 = 0, vtemp = 0 ;
          for(let i = 0; i < ds_ss.length ; i++){
            ...
            if (ix < 43)  {
              ...
              vtemp = rtags[a] / 100;
              con.query('INSERT INTO moteinfo ( mmgb, seq, stand, act, batt, rtd1,rtd2,rtd3,temp, tm ) value (?,?,?,?,?,?,?,?,?,?) ',
              ['2', ds_ss[i][6], ds_ss[i][2], ds_ss[i][0], ds_ss[i][3], vrtd1,vrtd2,vrtd3, vtemp, tm])
              .catch(err => console.error(err, ds_ss[i]));
            } 
          }
...
async function main()  {
  ...
  console.log("** 4PCM RM Roll 모니터링 start **") ;
  // setTimeout(main2_loop, 1000);

  // setTimeout( main_loop,  nextt - moment() ) ;
  setTimeout(main_loop, nextt - moment());
  setInterval(() => {
    con.query('INSERT INTO motehist (pkey, mmgb, seq, swseq, stand, batt, act, rtd1,rtd2,rtd3, temp, tm) \
